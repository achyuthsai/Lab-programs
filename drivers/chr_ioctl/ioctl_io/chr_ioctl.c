...
static long myioctl (struct file *fi, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		...
		case FILL_CHAR: if(_IOC_DIR(cmd) && _IOC_WRITE){
		...
		case SET_SIZE: if(_IOC_DIR(cmd) && _IOC_WRITE){
					if(access_ok(VERIFY_WRITE, (void *)arg, _IOC_SIZE(cmd)))
						return -EIO;
				}
				obj1.size = arg;
				krealloc(obj1.kbuf, arg, GFP_KERNEL);
				 pr_info("SET_SIZE invokde\n");
				pr_info("size: %d\n", obj1.size);
				break;
		case GET_SIZE: if(_IOC_DIR(cmd) && _IOC_READ){
					if(access_ok(VERIFY_READ, (void *)arg, _IOC_SIZE(cmd)))
						return -EIO;
				}

				copy_to_user(p, &obj1.size, sizeof(int));
				break;
		default: 
			return -1;
	}
